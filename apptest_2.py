...
def selectdf(q1):
	...
	if 'latitude' in q2_list and 'longitude' not in q2_list:
		q2_list.append('longitude')
	if 'longitude' in q2_list and 'latitude' not in q2_list:
		q2_list.append('latitude')	
...
def selectdf2(q1,q2,dfm):
	...
	if q1 in cat_cols and q2 in cat_cols:
		...
		if autre[0] in ['latitude','longitude']:
			autre=['latitude','longitude']
...
#st.write([i for i in Dico])

st.write(correl)
st.write([i for i in questions if 'school' in i])
...
def main():
	...
	#st.write(graphs)
	q1 = st.sidebar.selectbox('Main question:', [None]+[i for i in correl][170:])
	if q1 != None:
		df=selectdf(q1)
		q2_list=correl[q1]	
...
#Visualisation des 6 paramètres le splus importants pour la prédiction			
		...
		if st.sidebar.checkbox('Do you want to generate graphs with other potential correlated questions?'):	
			...
			for q2 in q2_list:
				...
				if q1 in ["longitude","latitude"] or q2 in ["longitude","latitude"]:
					
					if q1 in ["longitude","latitude"]:
						position,question=q1,q2
						quests1=["longitude","latitude"]
					else:
						position,question=q2,q1
						quests2=["longitude","latitude"]
					quest=quests1+quests2
					
					
					if question in cat_cols:
						df3=selectdf2(q1,q2,df.fillna(''))
						st.plotly_chart(px.scatter(df3, x="longitude", y="latitude", color=question))		
					else:
						st.plotly_chart(px.scatter(df.fillna(''), x="longitude", y="latitude", color=question))
				...
				# On regarde maintenant si une des deux est catégorielle	
					...
					if cat in []:	
						...
						for i in range(n):
							if i%2==0:
								if autre in continues:
									fig=px.scatter(df3, x=cat_quests[i], y=autre)
									fig.update_layout(xaxis={'title':cat_quests[i]},yaxis_title=autre)
									col1.plotly_chart(fig)
								else:
									col1.plotly_chart(box(cat_quests[i],autre,cat_quests[i],autre,df3))
							else:
								if autre in continues:
									fig=px.scatter(df3, x=cat_quests[i], y=autre)
									fig.update_layout(xaxis={'title':cat_quests[i]},yaxis_title=autre)
									...
									col3.plotly_chart(box(cat_quests[i],autre,cat_quests[i],autre,df3))
